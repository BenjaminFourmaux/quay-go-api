...
func GetManifestLabel(repositoryNamespaced string, manifestRef string, labelId string, currentUser *Auth.AuthenticatedUser) (Dto.ManifestLabel, error) {
	...
	if err != nil {
		...
		case "record not found":
			...
			return Dto.ManifestLabel{}, Errors.ManifestLabelNotFound(labelId, manifestRef, repositoryNamespaced)
		default:
	...
	// Convert model into dto
	labelModel := labelManifestModel.Label
	label := Dto.ManifestLabel{
		Id:         labelModel.UUID,
		Key:        labelModel.Key,
		Value:      labelModel.Value,
		SourceType: Common.MapLabelSourceType(labelModel.SourceTypeId).Name,
		MediaType:  Common.MapMediaTypeName(labelModel.MediaTypeId),
	}
